...
static Node_t* Clone(Node_t* list, std::unordered_map<Node_t*, Node_t*>& map) {
    if (list == NULL) {
        return NULL;
    }
    ...
    if (map.find(list) == map.end()) {
        ...
        map[list] = newNode;
        newNode->next = Clone(list->next, map);
        newNode->reference = Clone(list->reference, map);
    }
...
{
    std::unordered_map<Node_t*, Node_t*> map;

    return Clone(list,map);
}
